...
test('get_run tool tolerates both run_id and agent_name references', async () => {
  const manager = {
    async getRun(input) {
      assert.equal(input.run_id, 'run-1');
      assert.equal(input.agent_name, 'worker1');
      assert.equal(input.cwd, '/tmp/project');
      return {
        run_id: 'run-1',
...
test('run-targeted tools reject missing run references', async () => {
  await withServer(async (client) => {
    ...
    assert.equal(result.isError, true);
    assert.match(result.content[0].text, /Provide run_id or agent_name/);
  });
});
